    ...
    <?php foreach (getData2("SELECT * FROM `tbl_nav_heading` WHERE 1") as $key => $banner) { ?>
        <div class="grid grid-cols-[auto_minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)] items-center gap-4 px-4 py-3 hover:bg-gray-50 text-gray-800 rounded">
            ...
            <div class="flex space-x-2">
                <a href="nav-heading/edit/<?= $banner['id'] ?>" class="text-blue-500 hover:text-blue-600"><i class="fa-solid fa-pen"></i></a>
                <a href="nav-heading/delete/<?= $banner['id'] ?>" class="text-red-500 hover:text-red-600"><i class="fa-solid fa-trash"></i></a>
            </div>
        </div>
    <?php } ?>
</div>
