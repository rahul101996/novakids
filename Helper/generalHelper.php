...
function SendOrderMail($purchase_id)
{

    $OrderDetails = getData2("SELECT tbl_purchase.*, indian_states.name AS state_name FROM tbl_purchase LEFT JOIN  indian_states ON tbl_purchase.state = indian_states.id WHERE tbl_purchase.userid = '$_SESSION[userid]' AND tbl_purchase.id = '$purchase_id' ORDER BY tbl_purchase.id DESC LIMIT 1")[0];
